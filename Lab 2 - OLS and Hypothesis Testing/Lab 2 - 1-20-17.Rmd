...
Now let's build an OLS estimator from scratch to estimate the regression we just did.
...
t() transposes a matrix, which is basically a reflection using the diagonal as an axis, 
and is denoted by an apostrophe in matrix notation.
...
```{r, message=F}
## Make IV and DV matrices for this regression
